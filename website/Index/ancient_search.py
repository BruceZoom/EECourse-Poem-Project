...
class AncientPoemSearcher(object):
    ...
    def gushiwen_search(self, command_dict, target_range=None,
                        targets=('text', 'title', 'dynasty', 'author', 'label', 'yiwen'
                                 'shangxi', 'imgurl')):
        res = []
        # print command_dict, target_range
        querys = BooleanQuery()
        for key, value in command_dict.items():
            if key not in ['author', 'dynasty', 'label', 'title', 'content']:
                continue
            # print key, value
        ...
        totalDocs = self.searcher.search(querys, utils.MAX_RESULTS).scoreDocs
